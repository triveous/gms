...
class DoclingIngestionManager:
    ...
    def request_docling_document(
        ...
        if ai_document.transformed_file is None:
            ...
            self.save_docling_json(docling_dict, original_file, ai_document.name)
        else:
            frappe.log("Loading existing Docling Document")
    ...
    def save_docling_json(
        ...
        file_doc: File = frappe.new_doc("File")
        file_doc.file_name = f"{original_file.file_name}_docling.json"
...
class ChunkContextualizer:
    ...
    async def run(self):
        ...
        agent = Agent(
            model=f"google-gla:{self.model}",
            model_settings={
                "google_cached_content": cache_name,
        ...
        tasks = [
            asyncio.create_task(self.chunk(idx, agent, chunk, lock))
            for idx, chunk in enumerate(self.chunks)
        ]
        await asyncio.gather(*tasks)

    async def chunk(
        self, idx: int, agent: Agent, chunk: Document, lock: asyncio.Semaphore
    ):
        async with lock:
            frappe.log(f"Contextualizing chunk {idx + 1}")
            prompt = (
                self.prompt.replace("{{chunk}}", chunk.page_content)
                if self.prompt
                else f"""Here is the chunk we want to situate within the whole document <chunk> {chunk.page_content} </chunk> 
                Please give a short succinct context to situate this chunk within the overall document for the purposes of improving search retrieval of the chunk. 
                Answer only with the succinct context and nothing else.
                Esnure you are not missing the bigger picture of the main document and context can independently help situate the chunk
                """
            )

...
def fetch_grant_partners(grant_id, quarter_value, page=1, page_size=10, project_id=None, sort_by=None):
	...
	# Group partners by milestone
	partners_map = {}
	for p in partner_rows:
		mid = p["parent"]
		pid = p.get("partner")
		responsibility_str = p.get("responsibility") or ""
		responsibilities = [r.strip() for r in responsibility_str.split(",") if r.strip()]
		proj_id = milestone_project_map.get(mid)
		...
		partners_map.setdefault(mid, []).append(
			{
				...
				"project_id": proj_id,
				"project_name": project_map.get(proj_id),
			}
		)
	...
	# ----------------------------
	if sort_by == "title":
		# Sort all partners by title within each milestone
		for mid in partners_map:
			partners_map[mid].sort(key=lambda x: x["title"] or "")
...
def fetch_grant_contributors(grant_id, quarter_value, page=1, page_size=10, role_filter=None, sort_by=None, project_id=None):
	...
	if role_filter:
		filtered_contributor_rows = [c for c in all_contributor_rows if c.get("role") == role_filter]
	
	# ----------------------------
	# STEP 4: Count total and apply pagination
	# ----------------------------
	total_count = len(filtered_contributor_rows)
	offset = (page - 1) * page_size
	total_pages = (total_count + page_size - 1) // page_size if total_count > 0 else 0
	
	# Paginate the filtered results
	paginated_contributor_rows = filtered_contributor_rows[offset:offset + page_size]
	
	if not paginated_contributor_rows:
		return {
			"contributors_map": {},
			"all_roles": all_roles,
			"all_projects": all_projects,
	...
	# Fetch member titles and designations
	member_ids = {c.get("team_member") for c in paginated_contributor_rows if c.get("team_member")}
	member_title_map = {}
	...
	if member_ids:
		...
		member_designation_map = {m["name"]: m.get("designation") for m in members}
	...
	# Group contributors by milestone
	contributors_map = {}
	for c in paginated_contributor_rows:
		mid = c["parent"]
		...
		contributors_map.setdefault(mid, []).append(
			{
				...
				"role": c.get("role"),
				"designation": member_designation_map.get(mid_id),
				"project_id": proj_id,
				"project_name": project_map.get(proj_id),
			}
		)
	...
	# ----------------------------
	# STEP 5: Apply sorting if requested
	# ----------------------------
	if sort_by == "name":
		# Sort all contributors by name within each milestone
		for mid in contributors_map:
			contributors_map[mid].sort(key=lambda x: x["title"] or "")
...
def get_grant_projects_by_quarter(grant_id, quarter_value):
	print(
		"get_grant_projects_by_quarter called with grant_id:", grant_id, "and quarter_value:", quarter_value
	)
	"""
	...
	projects = frappe.get_all(
		"Grant Project",
		fields=["name", "title", "alias", "start_date", "end_date", "lead_organization"],
	...
	for project in projects:
		project["lead_organization"] = partner_map.get(project.get("lead_organization"), {})
	# ----------------------------
	# STEP 2: Fetch Milestones
	# ----------------------------
	milestones = frappe.get_all(
		"Grant Project Milestone",
		fields=["name", "project", "title", "period_start", "creation", "forecasted_amount"],
		filters={"project": ["in", project_ids]},
	...
	# ----------------------------
	milestone_map = {m["name"]: m for m in milestones if m["name"] in filtered_milestone_ids}
	...
	if milestone_ids:
		metric_rows = frappe.get_all(
			"Grant Metric Value",
			fields=["parent", "title", "type", "data_string", "data_int", "data_float", "data_boolean"],
	...
	for row in metric_rows:
		...
		if title == "Budget Spent":
			try:
				...
				numeric_value = 0
			budget_spent_per_milestone[mid] = budget_spent_per_milestone.get(mid, 0) + numeric_value
			total_budget_spent += numeric_value

	# ----------------------------
	# STEP 5: Partners & Contributors now fetched via separate API endpoints
	# ----------------------------
	# Call the separate API endpoints if needed
	# partners_result = fetch_grant_partners(grant_id, quarter_value, page=1, page_size=10)
	# contributors_result = fetch_grant_contributors(grant_id, quarter_value, page=1, page_size=10)
	...
	overall_utilization = (overall_spent / overall_forecasted) * 100 if overall_forecasted else 0
	return {
...
def compare_quarter_metrics_grant_forcast(grant_id, quarter_value, compare_with):
	...
	for m in all_milestones:
		...
		if curr_start <= ps <= curr_end:
			...
			curr_forecasted_amounts += float(m.get("forecasted_amount")) if m.get("forecasted_amount") else 0

		if prev_start <= ps <= prev_end:
			prev_m_ids.append(m["name"])
			prev_forecasted_amounts += float(m.get("forecasted_amount")) if m.get("forecasted_amount") else 0
...
@frappe.whitelist(allow_guest=True)
def compare_quarter_metrics_grant_project_spesific(grant_id, quarter_value, compare_with):
	...
	# ------------------------------------------------
	projects = frappe.get_all("Grant Project", fields=["name"], filters={"grant": grant_id})
	...
	for p in projects:
		...
		milestones = frappe.get_all(
			"Grant Project Milestone", fields=["name", "period_start"], filters={"project": project_id}
		)
		...
		project_results.append(
			{
				"project": project_id,
				"result": {"quarter": quarter_value, "compare_with": compare_with, "metrics": metric_result},
			}
		)
